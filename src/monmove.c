 ...
 */
static void remove_useless_spells(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p, u32b *f7p, bool los)
{
	monster_type *m_ptr = &m_list[m_idx];
	monster_race *r_ptr = &r_info[m_ptr->r_idx];
	...
	if (!(m_ptr->cdis > m_ptr->best_range) && los) f6 &= ~(RF6_TELE_SELF_TO);
 ...
 */
static int choose_attack_spell_fast(int m_idx, u32b *f4p, u32b *f5p, u32b *f6p, u32b *f7p, bool random)
{
	int i, num=0;
	byte spells[128];
...
static int cave_passable_mon(monster_type *m_ptr, int y, int x, bool *bash)
{
	...
	if (!(cave_info[y][x] & (CAVE_WALL)))
	{
		...
		if (feat == FEAT_WATER)
		{
			...
			if (r_ptr->flags4 & (RF4_BRTH_FIRE)) return (0);
			if (strchr("uU", r_ptr->d_char)) return (0);
			...
			/* Maybe water should slow terrestrial creatures down... */
		}
		...
		if (feat == FEAT_LAVA)
		{
			...
			if ((r_ptr->flags2 & (RF2_FLYING)) && (m_ptr->hp > 49))
				return (move_chance);
...
static void process_move(monster_type *m_ptr, int ty, int tx, bool bash)
{
	...
	if (cave_info[ny][nx] & (CAVE_WALL))
	{
		...
		else if (r_ptr->flags2 & (RF2_KILL_WALL))
		{
			...
			if (cave_info[ny][nx] & (CAVE_SEEN)) do_view = TRUE;
		}
...
static void process_monster(monster_type *m_ptr)
{
	...
	if ((m_ptr->csleep) || (m_ptr->stasis)) return;
	...
	if ((p_ptr->superstealth) && (!p_ptr->aggravate))
	{
		    ...
		    (rand_int(p_ptr->lev * 2 - r_ptr->level) != 0))
		{
			...
			if ((!m_ptr->ty) && (!m_ptr->tx)) random = TRUE;
		}
	}
